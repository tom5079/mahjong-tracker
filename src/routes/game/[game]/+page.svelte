<!-- @migration-task Error while migrating Svelte code: can't migrate `let error = ''` to `$state` because there's a variable named state.
...
<script lang="ts">
    ...
    async function onSubmitAction(
        token: string,
        action: PrismaJson.Action | PrismaJson.TimerAction
    ) {
        error = ''
    ...
    function undo() {
        window.grecaptcha.ready(() => {
            window.grecaptcha
                .execute(PUBLIC_CAPTCHA_CLIENT_KEY, { action: 'undo' })
                .then((token) => {
...
<main class="mx-auto max-w-lg px-4">
    ...
    {:else}
        ...
        {:else}
            {#if timer}
                <div
                    class="my-4 flex w-full flex-row items-center justify-between rounded-lg border p-8"
                >
                ...
                <section class="mx-auto max-w-lg">
                    {#if data.game.timer.state === 'running'}
                        ...
                        <div class="my-4 grid grid-cols-2 gap-x-8 gap-y-4">
                            <button
                                on:click={ron}
                                class="h-12 rounded bg-red-500 text-xl font-bold text-white"
                                >Ron</button
                            >
                            <button
                                on:click={tsumo}
                                class="h-12 rounded bg-blue-500 text-xl font-bold text-white"
                                >Tsumo</button
                            >
                            <button
                                on:click={draw}
                                class="h-12 rounded bg-gray-500 text-xl font-bold text-white"
                                >Draw</button
                            >
                            <button
                                on:click={chonbo}
                                class="h-12 rounded bg-yellow-500 text-xl font-bold text-white"
                                >Chonbo</button
                            >
                            <button
                                on:click={() => (action = { type: 'end' })}
                                class="h-12 rounded bg-green-600 text-xl font-bold text-white"
                                >End Game</button
                            >
                                ...
                                {#each state.match.result as result}
                                    ...
                                    <p class="flex flex-row items-center space-x-2">
                                        <span class="space-x-2 truncate rounded p-1">
                                            {state.players.find(
                                                (player) => player.user.id === result.player
                                            )?.user.username}
                                        </span>
                                        <span
                                            class:text-red-700={score < 0}
                                            class:text-blue-700={score > 0}
                        ...
                        {#each [...state.history].reverse() as history}
                            <div>
                                <div class="flex flex-row items-center justify-between">
                                    ...
                                    <p
                                        class="rounded-lg px-4 py-2 text-white {historyActions[
                                            history.action
                                        ][1]}"
                                    >
                                ...
                                <div class="grid grid-cols-2 gap-2">
                                    ...
                                    {#if history.result[3] != null}
                                        <HistoryEntry
                                            ...
                                            username={state.players.find(
                                                (player) =>
                                                    player.user.id === history.result[3].player
                                            )?.user?.username ?? 'unknown user'}
                                        />
                                    {/if}
                                </div>
                ...
                <section class="mx-auto max-w-lg">
                    ...
                    {:else}
                        <TsumoScoring
                            scores={state.players.find(
                                (player) =>
                                    action?.type === 'tsumo' && player.user.id === action.winner?.id
                            )?.wind === 0
                                ? data.game.event.ruleset.scores.dealer
                                : data.game.event.ruleset.scores.nonDealer}
                            onScore={(score) => {
            ...
            {:else if action.type === 'draw' || action.type === 'nagashi'}
                <section class="mx-auto max-w-lg">
                    ...
                    {:else}
                        <p class="w-full py-16 text-center text-2xl">
                            Select tenpai players from above
                        </p>
                    {/if}
                </section>
            {:else if action.type === 'chonbo'}
            ...
            {:else if action.type === 'oyaNagashi'}
                <section class="mx-auto max-w-lg">
                    ...
                    <p class="w-full py-16 text-center text-2xl">
                        Click submit to pass the dealership
                    </p>
                </section>
            {:else if action.type === 'undo'}
                <section class="mx-auto max-w-lg">
                    ...
                    <p class="w-full py-16 text-center text-2xl">
                        Click submit to undo last action
                    </p>
                </section>
            {/if}
        {/if}
